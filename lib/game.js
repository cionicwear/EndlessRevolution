...
class Game {
  ...
  openTutorialModal() {
    ...
    setTimeout(function(){ this.closeTutorialModal(); }.bind(this), 3000);
  }

  startTutorial() {
    this.inTutorial = true;
    const tutorialSteps = ['leftUp', 'leftSide', 'leftDown', 'rightUp', 'rightSide', 'rightDown']
    let step = 1;
    this.RcreateUpArrow(); // fire first arrow right away
    const myLoop = () => {         //  create a loop function
      setTimeout(function() {   //  call a 3s setTimeout when the loop is called
        if (tutorialSteps[step] === 'leftUp') {
          this.RcreateUpArrow();
        } else if (tutorialSteps[step] === 'leftSide') {
          this.RcreateLeftArrow();
        } else if (tutorialSteps[step] === 'leftDown') {
          this.RcreateDownArrow();
        } else if (tutorialSteps[step] == 'rightUp') {
          this.RcreateUpArrow();
        } else if (tutorialSteps[step] === 'rightSide') {
          this.RcreateRightArrow();
        } else if (tutorialSteps[step] === 'rightDown') {
          this.RcreateDownArrow();
        }
        step++;
