...
class Game {
  constructor(stage) {
    ...
    this.tutorialSteps = ['leftUp', 'leftSide', 'leftDown', 'rightUp', 'rightDown']
  ...
  LcreateLeftArrow() {
    ...
    function leftTick(event) {
      ...
      if (this.LleftArrows[0] && this.LleftArrows[0].y == LOWER_Y_BOUND) {
        // this.cionic.sendJSON(STEP_LABEL, { label: 'left_side', ts: Date.now() })
      }
  ...
  LcreateDownArrow() {
    ...
    function downTick(event) {
      ...
      if (this.LdownArrows[0] && this.LdownArrows[0].y == LOWER_Y_BOUND) {
        // this.cionic.sendJSON(STEP_LABEL, { label: 'left_back', ts: Date.now() })
      }
  ...
  LcreateUpArrow() {
    ...
    function upTick(event) {
      ...
      if (this.LupArrows[0] && this.LupArrows[0].y == LOWER_Y_BOUND) {
        // this.cionic.sendJSON(STEP_LABEL, { label: 'left_forward', ts: Date.now() })
      }
  ...
  RcreateDownArrow() {
    ...
    function downTick(event) {
      ...
      if (this.RdownArrows[0] && this.RdownArrows[0].y == LOWER_Y_BOUND) {
        // this.cionic.sendJSON(STEP_LABEL, { label: 'right_back', ts: Date.now() })
      }
  ...
  RcreateUpArrow() {
    ...
    function upTick(event) {
      ...
      if (this.RupArrows[0] && this.RupArrows[0].y == LOWER_Y_BOUND) {
        // this.cionic.sendJSON(STEP_LABEL, { label: 'right_forward', ts: Date.now() })
      }
  ...
  RcreateRightArrow() {
    ...
    function rightTick(event) {
      ...
      if (this.RrightArrows[0] && this.RrightArrows[0].y == LOWER_Y_BOUND) {
        // this.cionic.sendJSON(STEP_LABEL, { label: 'right_side', ts: Date.now() })
      }
  ...
  randomGen() {
    let randArrow;
    while (true) {
      randArrow = Math.floor(Math.random() * 7) + 1;
      // TODO: remove this when we wanna use right steps
      if (randArrow != this.lastArrow && randArrow != 4) break;
    }
    this.lastArrow = randArrow;

    switch(randArrow) {
      case 1: this.LcreateLeftArrow();  break;
      case 2: this.RcreateDownArrow();  break;
      case 3: this.RcreateUpArrow();    break;
      case 4: this.RcreateRightArrow(); break;
      case 5: this.LcreateLeftArrow();  break;
      case 6: this.LcreateDownArrow();  break;
      case 7: this.LcreateUpArrow();    break;
      // TODO: center steps
    }
  }
  ...
  startTutorial() {
    ...
    let step = 1;
    this.LcreateUpArrow(); // fire first arrow right away
    const myLoop = () => {         //  create a loop function
      this.tutorialLoop = setTimeout(function() {   //  call a 3s setTimeout when the loop is called
        if (this.tutorialSteps[step] === 'leftUp') {
          ...
          this.LcreateLeftArrow();
        } else if (this.tutorialSteps[step] === 'leftDown') {
          this.LcreateDownArrow();
