...
class Game {
  ...
  LcreateDownArrow() {
    ...
    function downTick(event) {
      ...
      if (this.RdownArrows[0] && this.RdownArrows[0].y == LOWER_Y_BOUND) {
        this.cionic.sendJSON(STEP_LABEL, { label: 'left_back_step', ts: Date.now() })
      }
  ...
  LcreateUpArrow() {
    ...
    function upTick(event) {
      ...
      if (this.RupArrows[0] && this.RupArrows[0].y == LOWER_Y_BOUND) {
        this.cionic.sendJSON(STEP_LABEL, { label: 'right_forward_step', ts: Date.now() })
      }
  ...
  RcreateDownArrow() {
    ...
    function downTick(event) {
      ...
      if (this.RdownArrows[0] && this.RdownArrows[0].y == LOWER_Y_BOUND) {
        this.cionic.sendJSON(STEP_LABEL, { label: 'right_back_step', ts: Date.now() })
      }
