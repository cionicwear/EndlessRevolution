...
class Game {
  constructor(stage) {
    ...
    this.music = new Audio('./assets/songs/touch.mp3');
  ...
  play(difficulty) {
    ...
    let that = this;
    this.music.addEventListener("ended", that.gameOver)
    this.music.play();

    // new arrow every 2 seconds
    this.myInt = setInterval( randomGen, 2000);
  ...
  check(arrows, direction) {
    ...
    if (arrows[0] && arrows[0].y < 160 && arrows[0].y > 40) {
      if (this.life > 0) {
        this.hit("excellent");
      }
